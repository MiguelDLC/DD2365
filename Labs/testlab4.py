...
#         return x[0] >= 0.2 and x[0] <= 0.6 and x[1] >= 0.45 and x[1] <= 0.55
# 
# # Define 2D geometry
# domain = Rectangle(Point(0.0, 0.0), Point(lx, ly))
# domain.set_subdomain(1, Rectangle(Point(0.2, 0.45), Point(0.6,0.55)))
# 
# 
# # Generate and plot mesh
# mesh2d = generate_mesh(domain, 32)
# plot(mesh2d, "2D mesh")
# 
...
amp_x = 0.0; amp_y = 0.3; freq = 0.1
kwargs = {"L" : L, "H" : H, "amp_x" :amp_x, "amp_y":amp_y, "freq":freq}
deform = [E, kwargs]

E = ("(x[0]-xc)/w/cosh((x[0]-xc)/w)*(x[0]*(1-x[0]/L))*(sin(2.0*pi*t*freq)+1e-2)*amp_x*x[1]/H*(1-x[1]/H) - (x[0]-xc)/w/cosh((x[0]-xc)/w)*(x[0]*(1-x[0]/L))*sin(2.0*pi*(t-dt)*freq)*amp_x*x[1]/H*(1-x[1]/H)",
     "- (x[1]-yc)/w/cosh((x[1]-yc)/w)*(x[1]*(1-x[1]/H))*(sin(2.0*pi*t*freq)-1e-2)*amp_y*exp(-(x[0]-xc)*(x[0]-xc)) + (x[1]-yc)/w/cosh((x[1]-yc)/w)*(x[1]*(1-x[1]/H))*sin(2.0*pi*(t-dt)*freq)*amp_y*exp(-(x[0]-xc)*(x[0]-xc))")
...
u1, p1, time, drag, lift, dt, psi = simulate(mesh, Tmax=0.1, meshdeform=deform)

# %%
u1, p1, time0, drag0, lift0, dt, psi = simulate(mesh, uinit=u1, pinit=p1, tinit=time[-1], Tmax=0.5, psi=psi, meshdeform=deform)
...
plt.plot(time, 0.3*np.sin(2.0*np.pi*time*freq-0.5*np.pi), label="Circle position")
plt.legend()
# %%
